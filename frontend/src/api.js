...
export function buildApi(authRequest) {
  return {
    ...
    listMindMaps: () => authRequest("/mind-maps"),
    createMindMap: (payload) => authRequest("/mind-maps", { method: "POST", body: JSON.stringify(payload) }),
    patchMindMap: (id, payload) => authRequest(`/mind-maps/${id}`, { method: "PATCH", body: JSON.stringify(payload) }),
    deleteMindMap: (id) => authRequest(`/mind-maps/${id}`, { method: "DELETE" }),
    listRoadmaps: () => authRequest("/roadmaps"),
    createRoadmap: (payload) => authRequest("/roadmaps", { method: "POST", body: JSON.stringify(payload) }),
    patchRoadmap: (id, payload) => authRequest(`/roadmaps/${id}`, { method: "PATCH", body: JSON.stringify(payload) }),
    deleteRoadmap: (id) => authRequest(`/roadmaps/${id}`, { method: "DELETE" }),
    importRoadmaps: (payload) => authRequest("/roadmaps/import", { method: "POST", body: JSON.stringify(payload) }),
